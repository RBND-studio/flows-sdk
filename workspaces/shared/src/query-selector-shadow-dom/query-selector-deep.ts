/* eslint-disable -- disable lint check */
// @ts-nocheck -- disable type check
// More info at: https://github.com/webdriverio/query-selector-shadow-dom
// version: 1.0.1
...
e.g.
*/
export function querySelectorAllDeep<E extends Element = Element>(
  selector,
  root = document,
  allElements = null,
): NodeListOf<E> {
...
export function querySelectorDeep<E extends Element = Element>(
  selector,
  root = document,
  allElements = null,
): E | null {
  ...
  if (document.head.createShadowRoot || document.head.attachShadow) {
    ...
    return selectionsToMake.reduce(
      (acc, minimalSelector) => {
        ...
        const lastSplitPart =
          splitSelector[possibleElementsIndex][splitSelector[possibleElementsIndex].length - 1];
