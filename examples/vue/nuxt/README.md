...
In [`plugins/flows.ts`](./app/plugins/flows.ts) you can find Flows plugin being initialized during `app:mounted` hook in the browser. In [`app.vue`](./app/app.vue) - `<flows-floating-blocks>` is added to render floating components. Because `@flows/js-components` is using HTML custom elements for rendering, we need to add `isCustomElement` in [`nuxt.config.ts`](./nuxt.config.ts).
